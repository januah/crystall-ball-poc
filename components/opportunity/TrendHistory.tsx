  ...
  return (
    <div className="space-y-2">
      {history.map((entry, i) => {
        ...
        return (
          <div key={entry.date} className="flex items-center gap-4 py-2.5 border-b border-border last:border-0">
            <span className="text-xs text-muted-foreground w-24 shrink-0">{entry.date}</span>
            <span className="text-sm text-foreground w-8 shrink-0">#{entry.rank}</span>
            {rankDelta > 0 ? (
            ...
            <span className="text-sm font-bold text-foreground w-16 text-right">{entry.score}/100</span>
          </div>
        );
      })}
    </div>
